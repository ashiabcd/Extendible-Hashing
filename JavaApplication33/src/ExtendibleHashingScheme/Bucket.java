...
class Bucket{
	...
	protected void insertKey(int key){
		try {
			...
			System.out.println("Please choose appropriate keys or change the blocking factor.");
		}
	}
	...
	private void distributeKeys(Bucket newBucket, int hashComputer){
		...
		while(oldKeysIt.hasNext()){
			...
			String keyHash = Bits.trim(Bits.generateBitString(hashedKey), this.localDepth);
